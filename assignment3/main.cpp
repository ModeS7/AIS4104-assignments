...
{
    double L1 = 0.2436, L2 = 0.2132, H1 = 0.1519, H2 = 0.0854, W1 = 0.1311, W2 = 0.0921;
                                                ...
                                                double gamma = 1e-2, double v_e = 4e-3, double w_e = 4e-3)
{
    ...
    Eigen::Matrix4d t = (t_sb).inverse() * t_sd;
    Eigen::VectorXd v_b;
    double theta;
    std::tie(v_b, theta) = math::matrix_logarithm(t);
    v_b *= theta * math::deg_to_rad;
    size_t iterations = 0;
...
Eigen::Matrix4d t_sd = transformation_matrix(math::rotation_matrix_from_euler("zyx", zyx), pos);
